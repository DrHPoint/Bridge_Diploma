...
describe("Hermes", function () {
  ...
  beforeEach(async () => {
    ...
    NFT = await ethers.getContractFactory("Standart_ERC721");
    nft = await NFT.connect(owner).deploy("Doctor", "WHO");
    BSC = await ethers.getContractFactory("Standart_ERC721");
    bsc = await BSC.connect(owner).deploy("Doctor", "WHO");
    Bridge1 = await ethers.getContractFactory("Bridge_ERC721");
    bridge1 = await Bridge1.connect(owner).deploy(nft.address, 1);
    Bridge2 = await ethers.getContractFactory("Bridge_ERC721");
  ...
  describe("Bridges", () => {
    it("Bridges", async () => {
      ...
      const ChairePerson1 = await bridge1.connect(owner).setChairePerson(addr3.address);
      const ChairePerson2 = await bridge2.connect(owner).setChairePerson(addr3.address);
